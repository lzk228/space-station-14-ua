...
class FluentSerializedMessage:
    ...
    def from_lokalise_keys(cls, keys: typing.List[LokaliseKey]):
        ...
        for key in keys:
            ...
            key_value = key.get_translation('uk').data['translation']
            key_attributes = []
            ...
            full_message = cls.from_yaml_element(key_name, key_value, key_attributes, key.get_parent_key(), Tuke)
    ...
    def get_attr(k, name, parent_id = None):
        if parent_id:
            ...
            return k.get_translation('uk').data['translation']
    ...
    def to_serialized_message(string_message):
        ...
        serialized = FluentSerializer(with_junk=Tuke).serialize(ast_message)
