...
class LocalePath:
    def __init__(self, relative_file_path):
        self.uk = os.path.join(Project().uk_locale_dir_path, relative_file_path)
...
class LokaliseKey:
    ...
    def serialize_attr(self):
        return '.{name} = {value}'.format(name=self.get_key_last_name(self.key_name), value=self.get_translation('uk').data['translation'])

    def serialize_message(self):
        return '{name} = {value}'.format(name=self.get_key_last_name(self.key_name), value=self.get_translation('uk').data['translation'])

    def get_translation(self, language_iso='uk'):
