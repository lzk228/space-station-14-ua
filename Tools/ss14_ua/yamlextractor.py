...
class YAMLExtractor:
    ...
    def execute(self):
        for yaml_file in yaml_files:
            ...
            uk_fluent_file_path = self.create_uk_fluent_file(en_fluent_file_path)
    ...
    def create_en_fluent_file(self, relative_parent_dir, file_name, file_data):
        ...
        logging.info(f'Актуалізовно файл англійської локалі {en_fluent_file.full_path}')

        return en_fluent_file.full_path

    def create_uk_fluent_file(self, en_analog_file_path):
        uk_file_full_path = en_analog_file_path.replace('en-US', 'uk-UA')

        if os.path.isfile(uk_file_full_path):
            return
        else:
            en_file = FluentFile(f'{en_analog_file_path}')
            file = FluentFile(f'{uk_file_full_path}')
            file.save_data(en_file.read_data())
            logging.info(f'Створено файл української локалі {uk_file_full_path}')

        return uk_file_full_path
...
logging.info(f'Пошук yaml-файлів ...')
